#!/usr/bin/env bash
# ==============================================================================
# LazyVPS v1.3.5 Main Menu Hotfix
# 功能：真正修改 GitHub 根目录 lazy-vps-menu.sh，把 AnyTLS / TUIC 接入主菜单。
# 用法：在 VPS- 仓库根目录执行：bash lazy-vps-mainmenu-hotfix-v1.3.5.sh
# ==============================================================================
set -Eeuo pipefail
...
[[ -f "$MAIN" ]] || { err "找不到 $MAIN。请先进入 GitHub 仓库根目录，也就是能看到 lazy-vps-menu.sh 的目录。"; exit 1; }
[[ -s "$MAIN" ]] || { err "$MAIN 是空文件，停止。"; exit 1; }
[[ -f "$ADDON" ]] || warn "当前目录没有 $ADDON；请把本包里的 lazy-vps-protocol-addon.sh 一起复制到仓库根目录。"
...
BRIDGE = r'''
# ==============================================================================
# LazyVPS v1.3.5 Main Menu Runtime Bridge / AnyTLS + TUIC 接入层
# 说明：本段插入在最终 dispatch 之前，不覆盖原 Trojan/VLESS/Hysteria2/Xray 逻辑；
# 原功能由 orig_lazyvps_run_choice / orig_lazyvps_quick 原样转发。
# ==============================================================================
LAZYVPS_PROTOCOL_BRIDGE_VERSION="v1.3.5"
VER="正式 v1.3.5 · V4/V6 双栈 + AnyTLS/TUIC 主菜单整合版"
...
if declare -p ITEMS >/dev/null 2>&1 && ((${#ITEMS[@]} >= 7)); then
  ITEMS[6]="Protocol Suite / Hysteria2 + AnyTLS + TUIC"
fi
if declare -p DESCS >/dev/null 2>&1 && ((${#DESCS[@]} >= 7)); then
...
# 删除旧 bridge，避免重复插入。
s = re.sub(r'\n# ={10,}\n# LazyVPS v1\.3\.[0-9]+ Main Menu Runtime Bridge.*?# ={10,}\s*\n', '\n', s, flags=re.S)
s = re.sub(r'\n# ={10,}\n# LazyVPS v1\.3\.[0-9]+ Protocol Addon Bridge.*?# ={10,}\s*\n', '\n', s, flags=re.S)

# 更新头部版本文字。
s = re.sub(r'Formal Version:\s*v[0-9.]+', 'Formal Version: v1.3.5', s, count=1)
s = re.sub(r'Update Date:\s*[0-9-]+', 'Update Date: 2026-07-04', s, count=1)
s = re.sub(r'VER="[^"]*"', 'VER="正式 v1.3.5 · V4/V6 双栈 + AnyTLS/TUIC 主菜单整合版"', s, count=1)
...
# 插入在最终 dispatch 之前，确保原数组、函数已经定义好。
markers = [
    'if [[ "${1:-}" == "--preview" ]]',
    "if [[ '${1:-}' == '--preview' ]]",
    'need_root\nif [[ "${1:-}" == "--quick" ]]',
...
if 'LazyVPS v1.3.5 Main Menu Runtime Bridge' not in s:
    s = s[:idx] + BRIDGE + '\n' + s[idx:]
...
if [[ -f "$ADDON" ]]; then
  if bash -n "$ADDON"; then
    ok "协议扩展语法检查通过：$ADDON"
  else
    err "协议扩展语法检查失败：$ADDON"
    exit 1
  fi
fi

if grep -qE 'v1\.3\.5|AnyTLS|TUIC|protocol_suite' "$MAIN"; then
  ok "关键词检查通过：v1.3.5 / AnyTLS / TUIC 已进入 lazy-vps-menu.sh"
else
...
if command -v git >/dev/null 2>&1 && git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  ...
  echo
  info "请确认上面必须出现：M lazy-vps-menu.sh"
fi

echo
ok "LazyVPS 主菜单已修正到 v1.3.5。"
echo "验证命令："
echo "  grep -nE 'v1.3.5|AnyTLS|TUIC|protocol_suite' $MAIN | head -40"
