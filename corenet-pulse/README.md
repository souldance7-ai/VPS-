...
- 原創機甲漫畫介面：白色裝甲、深藍指揮台、紅黃識別色、網點與斜切面板，保留清楚的即時數值；
- 原生 SVG 機器人動畫與 GIF 素材，支援「暫停動畫」及系統減少動態效果設定；
- 卡片／列表切換、搜尋、狀態／地區／服務商篩選、排序與 12／24／50／100 筆分頁；
- 右上角動態顯示選單，支援機甲白、海岸青、夜航藍、霓虹莓、夕照金五套色系，以及繁體中文、簡體中文、English 即時切換；
- 常用節點收藏與僅看收藏、手動立即更新、資料更新時間與過期提示，偏好存於本機瀏覽器；
...
公開頁面右上角的「顯示設定」採用帶過渡動畫的浮動選單。色系與語言切換均在瀏覽器本機完成，不會傳送到 Hub，也不會改動節點設定。選擇會存於瀏覽器 `localStorage`，下次開啟仍沿用；清除網站資料即可恢復預設的機甲白與繁體中文。舊版已保存的色系仍會保留，可在選單切換到機甲白。

- 色系：機甲白、海岸青、夜航藍、霓虹莓、夕照金；
- 語言：繁體中文、簡體中文、English；
- 無障礙：完整鍵盤焦點、`Escape` 收合選單，並遵循 `prefers-reduced-motion`；
- 響應式：桌面顯示文字按鈕，手機改為精簡圖示，支援 320 px 寬度。

機器人與裝甲圖形為本專案原創向量素材，沒有使用官方角色圖片或商標。SVG 在網頁保持清晰，GIF 可供分享使用；所有靜態素材隨 Go Hub 一起嵌入，不依賴第三方圖片服務。動畫不代表線路健康、頻寬或測速結果；在線率、資源與上海／安徽三網指標均來自實際 Agent 回報，未收到資料時保持待接入或待測試。
...
腳本從原始碼建置最新版、備份目前程式、原子替換 Hub 並重啟；若健康檢查失敗會恢復上一版。檢查包括服務狀態、公開 API、原有節點 ID，以及實際頁面和所引用的 JS／CSS／圖片與本次原始碼是否一致，不再綁定舊版頁面版本字串。若沒有 Go 1.22+，會從 Go 官方下載臨時工具鏈並驗證 SHA-256；完成後移除，已有的系統 Go 保持不變。

沿用原有節點設定、Token、管理登入設定、自訂節點名稱、探測目標與 Cloudflare Tunnel。原 Agent 的資源回報可繼續使用；要新增三網 Ping，須依下方指令更新 Agent。Hub 重啟會清空記憶體中的短期圖表，各 Agent 會在下一次回報時重新顯示在線。完成後以 `Ctrl+F5` 重新整理網頁。更新只需在原 Hub 執行一次，不需在每台節點重裝 Hub。

需要固定版本時，把下載網址中的 `main` 替換為完整 commit SHA，並用 `PULSE_REF=同一個SHA bash /tmp/pulse-update-hub.sh` 執行。上一版程式保存在 `/var/backups/corenet-pulse/`，更新結束會顯示其完整路徑。
...
本版修正 Ping 的封包計數：舊版同時使用 `-c 3` 與 `-w 5`，iputils 遇到丟包時可能繼續送出超過三個封包，導致有效的無回應樣本被判為「無法測試」、未計入丟包率。新版固定每輪最多三個封包，由 Go 控制整輪時間上限。此修正需要更新各節點的 Agent；只更新 Hub 外觀不會改變遠端 Ping 行為。更新後重新累積最近十分鐘樣本，不回填先前未成功記錄的測量。
...
[MIT](LICENSE)。參考專案僅作產品方向研究，未複製其原始碼或視覺資產。機甲介面及向量／動畫素材為原創設計，並非任何動畫系列的官方網站或授權角色素材。

### 前端回归检查

安装 Playwright 与 Chromium 后，在 `corenet-pulse` 目录执行 `node scripts/test-ui.cjs`。也可通过 `PULSE_BROWSER_EXECUTABLE=/path/to/chromium` 选择已有浏览器。脚本启动临时本机服务，使用隔离测试数据检查收藏、排序、三语切换、动画暂停、离线重连及错误数据保护，不访问正式探针。

原生 SVG 为首页动画；GIF 为轻量动画素材，两者均为本项目原创。动画可暂停，系统“减少动态效果”偏好会默认启用静态图。素材源与重制脚本一并开放：`scripts/render-mecha-assets.py`。
