...
APP="懒人建 VPS 快速菜单包"
VER="正式 v1.3.8 · Protocol Suite / AnyTLS + TUIC 主入口版"
...
install_addon_if_missing(){
  ...
  if addon="$(find_addon 2>/dev/null)"; then
    if bash -n "$addon" >/dev/null 2>&1; then
      echo "$addon"; return 0
    fi
    warn "找到 lazy-vps-protocol-addon.sh，但语法检查失败，准备重新下载：$addon"
  fi

  warn "本地未找到可用的 lazy-vps-protocol-addon.sh，尝试从 GitHub 下载。"
  if has curl; then
    mkdir -p "$ROOT"
    ...
    if curl -fL --retry 2 --connect-timeout 10 "$REPO_RAW/lazy-vps-protocol-addon.sh?v=$(date +%s)" -o "$tmp"; then
      ...
      rm -f "$tmp"
      err "GitHub 下载到的 addon 语法检查失败；请确认 GitHub 根目录 lazy-vps-protocol-addon.sh 已覆盖为 v1.3.8。"
      return 1
    fi
    rm -f "$tmp"
  fi
  err "找不到 lazy-vps-protocol-addon.sh。请把它与 lazy-vps-menu.sh 一起上传到 GitHub 根目录，或放到 /opt/lazy-vps-menu/。"
  return 1
}
...
run_legacy(){
  ...
  if legacy="$(install_legacy_if_missing 2>/dev/null)"; then
    ...
    echo
    echo "解决方式：在 GitHub 仓库根目录先执行 patch-replace-main-v1.3.8.sh。"
